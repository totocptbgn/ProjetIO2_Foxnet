{
  ...
  class Robot {
    ...
    update() {
      ...
      for (const point of this.points) {
        ...
        // ---- dance ----
        if (this !== dancerDrag) {
          point.fn && point.fn(45 * Math.sqrt(this.size), this.dir);
        }
        // ---- verlet integration ----
  ...
  Robot.Link = class Link {
    constructor(parent, p0, p1, dist, size, light, force, disk) {
      ...
      this.image = stroke(
        "hsl(" + parent.color + " ,90%, " + parent.light * this.light + "%)",
        true
      );
  ...
  // ---- instanciate robots ----
  for (let i = 0; i < 1; i++) { // Big One
    dancers.push(
      new Robot(
        220, // color
        10, // light
        (window.location.href.indexOf("fullcpgrid") > -1) ? 3 : 6, // size
        (i + 4.5) * canvas.width / 9, // x
        canvas.height * ground - 300, // y
        struct // struct
      )
    );
  }
  for (let i = 0; i < 3; i++) { // Middle ones
    dancers.push(
      new Robot(
        i * 360 / 7,
        40,
  ...
  for (let i = 0; i < 20; i++) { // Lil one
    dancers.push(
      new Robot(
        i * 360 / 20, // color
        50, // light
        (window.location.href.indexOf("fullcpgrid") > -1) ? 3 : 1, // size
        (i + 0.5) * canvas.width / 20, // x
        canvas.height * ground - 300, // y
        struct // struct
      )
    );
